  ...
  return `
    <div class="product-card" onclick="location.href='/product.html?id=${p.id}'">
      <div class="product-img-wrap">
        <img src="/images/${p.image}" alt="${p.name}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
...
async function addToCartFromCard(productId) {
  if (!API.isLoggedIn()) { toast('Please login to add items to cart', 'error'); setTimeout(() => location.href = '/login.html', 1200); return; }
  try {
    ...
    <nav class="navbar">
      <div class="nav-inner">
        ...
        <div class="nav-search">
          <input type="text" id="nav-search-input" placeholder="Search medicines, health products..."
        ...
        <div class="nav-actions">
          ${isLoggedIn ? `
            ...
            <span class="nav-btn" style="cursor:default">Hi, ${user?.name?.split(' ')[0] || 'User'}</span>
            <a class="nav-btn" href="/cart.html" style="position:relative">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
              Cart
              <span class="cart-badge" id="cart-badge" style="display:none">0</span>
            </a>
            <button class="nav-btn btn-login-nav" onclick="handleLogout()">Logout</button>
          ` : `
            <a class="nav-btn" href="/">Home</a>
  ...
  return `
    <footer>
      <p style="margin-bottom:0.5rem">
        <strong style="color:white">MediShop</strong> — Your trusted online pharmacy
      </p>
